...
router.get('/'+version+'/tasks/randomise', function(req, res) {
    ...
    req.session.data['OtherApplicationsPartner'] = '';
    req.session.data['PersonalDetails'] = '';
    req.session.data['PersonalDetailsPartner'] = '';
...
router.post('/' + version + '/tasks/other-applications-partner', function(req, res) {
    // if(req.session.data['telephone']=='yes' && req.session.data['IDOC']=='yes' && req.session.data['online']=='yes'&& req.session.data['paper']=='yes'){
    //     req.session.data['OtherApplicationPartnerError'] = 'false';
        if(req.session.data['OtherApplicationsPartner'] == 'yes') {
            res.redirect("dropout")
        } 
        else{
            res.redirect("tasklist")
...
router.post('/' + version + '/tasks/personal-details-partner', function(req, res) {
    if(req.session.data['taskType']=='pay')
        if(req.session.data['PersonalDetailsPartner'] == 'yes') {
            res.redirect("nationality-partner")
        } 
        else{
            res.redirect("dropout")
        }
...
router.post('/v27/tasks/non-dependents', function(req, res) {
...
router.post('/' + version + '/tasks/nationality-partner', function(req, res) {
    //get six month date
    var immigrationDate = new Date()
    immigrationDate.setMonth(immigrationDate.getMonth() - 6)
   var immigrationDateString = immigrationDate.getDate() + " " + months[immigrationDate.getDay()-1] + " " + immigrationDate.getFullYear()
   req.session.data['immigrationDateString'] = immigrationDateString
    ...
    if(req.session.data['nationalityCheckPartner'] == 'Something else') {
...
router.post('/' + version + '/tasks/confirm-nationality-partner', function(req, res) {
    if(req.session.data['confirmNationalityPartner'] == 'yes') {
        res.redirect("immigration-status-partner")
    } 
    else{
        res.redirect("dropout")
    }
